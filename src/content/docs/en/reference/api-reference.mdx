...
The current route pattern being rendered. The pattern will look like a file path, without the [`srcDir`](/en/reference/configuration-reference/#srcdir) and the `pages` folder. The path will start with a leading slash, and it won't have the file extension.

For example, if the current route is `src/pages/en/blog/[slug].astro`, then `Astro.routePattern` will be `/en/blog/[slug]`.
...
For example, if the current route is `src/pages/api/insert.ts`, then `Astro.routePattern` will be `/en/blog/insert`.
